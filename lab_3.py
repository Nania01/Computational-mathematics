...
f_1 = lambda x: x**2 + log(x)
f_2 = lambda x: x**2 - sin(x)
f_3 = lambda x: x**2 - 0.5 * exp(-x)

f_1_first_diff = lambda x: 2*x + 1/x
f_2_first_diff = lambda x: 2*x - cos(x)
func_3_first_diff = lambda x: 2*x + 0.5 * exp(-x)

f_1_second_diff = lambda x: 2 - 1/(x**2)
f_2_second_diff = lambda x: 2 + cos(x)
f_3_second_diff = lambda x: 2 - 0.5 * exp(-x)

x_point = 0.55
nodes = [0.5, 0.55, 0.6]

results = [
    ("x^2 + ln(x)", 1, find_first_diff(nodes, x_point, f_1), f_1_first_diff(x_point)),
    ("x^2 - sin(x)", 1, find_first_diff(nodes, x_point, f_2), f_2_first_diff(x_point)),
    ("x^2 - 0.5 * exp(-x)", 1, find_first_diff(nodes, x_point, f_3), func_3_first_diff(x_point)),
    ("x^2 + ln(x)", 2, find_second_diff(nodes, f_1), f_1_second_diff(x_point)),
    ("x^2 - sin(x)", 2, find_second_diff(nodes, f_2), f_2_second_diff(x_point)),
    ("x^2 - 0.5 * exp(-x)", 2, find_second_diff(nodes, f_3), f_3_second_diff(x_point)),
]
